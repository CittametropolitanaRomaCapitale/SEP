...
export const useRispondiConProtocollo = () => {
  ...
  const rispondiConProtocollo = async (emailData) => {
    const queryParams = {
      ...tableData?.filters,
    ...
    dispatch(setinoltraRispondiConProtocollo(dictionary.get('actionRispondiConProtocollo')));
    dispatch(setEmailData(emailData));
    await push({ pathname: '/pec', query: queryParams })
    push({ pathname: `/crea-protocollo` })
  };
  return { rispondiConProtocollo };
};