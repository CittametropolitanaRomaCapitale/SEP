...
const StyledTable = styled(MUITable)(({ theme }) => ({
  '& .MuiTableRow-root': {
    ...
    borderBottom: `1px solid ${theme.palette.mode === 'light' ? '#E6E8F0' : '#303030'
      }`,
  },
}));
  ...
  (
    ...
    useEffect(() => {
      if (onSelectRow && !loading) {
        ...
        const filteredRows = updatedSelectedRows.filter(row =>
          !currentPageIds.has(row.id.toString()) || rowSelection[row.id.toString()]
        );
    ...
    const [pagination, setPagination] = useState({
      pageIndex: 0,
      pageSize: defaultPageSize > 0 ? defaultPageSize : 10,
    });
    ...
    return (
      <MUITableContainer>
        <StyledTable>
          ...
          {loading && (
            <SkeletonTableBody skeletonRows={skeletonRows} columnsCount={tableColumns.length} />
          )}
