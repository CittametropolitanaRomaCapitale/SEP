...
const RiceviDeleghe: FC = () => {
  ...
  const { isOpen, close, content, openWithContent } = useDialog({
    dialog_id: `cancellaAllegatoRicevi_${defaultValues?.id}`
  });
              ...
              onUpload: () =>
                dispatch(
                  SSOApi.util.invalidateTags([
                    { type: 'UserByUserIdDelegations' as never }
                  ])
                )
            });
          }
  ...
  const onDownloadAllegato = (id) => {
    window.open(
      `${process.env.NEXT_PUBLIC_API_URL}/api/docs/download_attachment/${id}`,
      '_blank'
    );
  };
  ...
  return (
    ...
    >
      ...
      <Grid item>
        <Allegati
          inputKey="riceviDelegheAllegati"
          disabled={false}
